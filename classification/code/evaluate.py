...
#     cm = confusion_matrix(np_y, np_pred_class)
    ...
    plot_conf_matrix(cm, classes=CLASSES,
                     normalize=False, title='Normalized confusion matrix') # k, 
...
#     print(y_one_hot.shape, y_pred.detach().cpu().numpy().shape)
    ...
    plt.legend()
    plt.savefig('../out_fig/comp_resnet_ROC_'+'.png') #+str(k)
#     plt.show()

    return accu, auc


def plot_conf_matrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues): #k,
    """
...
#         print(i, j)
    ...
    plt.savefig('../out_fig/comp_resnet_confusion_matrix_'+'.png') # +str(k)
# ——————————————————————————————————————————————————————————————————
...
#         print(k)
    model_load_path = '../model/res18_epoch/model_res18' # + str(k)
    # load model weight
...
#     accu[k-1], auc[k-1] = evaluate_model(test_pred_eval, labels_test_eval, k)
    accu_comp, auc_comp = evaluate_model(test_pred_eval, labels_test_eval) # , k

    
print("The compression accuracy is %d, and the auc is %d " % (accu_comp, auc_comp))
...
# plt.savefig('../out_fig/comp_resnet_TestAccuracy.png')
# # plt.show()
...
# plt.ylabel('AUC')
# plt.title('Test AUC')
# plt.savefig('../out_fig/comp_resnet_AUC.png')
# # plt.show()
print('-----------------------------------------------------------------------------------------')
